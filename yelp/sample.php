...
if($current_hour >= 5 && $current_hour < 10){
	...
	$_GET['keywords'] = 'hiking restaurant';
...
if($current_hour >= 15 && $current_hour < 20){

	$_GET['keywords'] = 'dinner';
	 	
}

if($current_hour >= 20){

	$_GET['keywords'] = 'sports bar';
...
function search($term, $location) {
    ...
    $url_params['term'] = $_GET['keywords'];
    
    $url_params['location'] = $_GET['zip'];
    
    $url_params['limit'] = $GLOBALS['SEARCH_LIMIT'];
    
    $url_params['sort'] = 1;

    $url_params['radius_filter'] = 2700;    
...
$yelp_results = query_api($term, $location);

//echo '<pre>'.print_r($yelp_results, TRUE).'</pre>';
